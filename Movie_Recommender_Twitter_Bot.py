    ...
    for mention in reversed(mentions):
        ...
        if mention.user.screen_name != 'KapilCh23582972':
            if 'brodad' in mention.full_text.lower():
                ...
                print('responding back...', flush=True)
                mention.full_text = mention.full_text.replace('@KapilCh23582972','').strip()
                final_user_ids, n_users, n_items, known_positives = find_user_id(model, data, mention.full_text)
                sample_recommendation(model, data, final_user_ids, n_users, n_items, known_positives, mention)
